...
contract("ZombieFeeding 1", function(accounts) {
  ...
  describe("Test req ZFE_1 (false)", function() {
    ...
    it("Test req ZFE_1 (true)", async function() {
      await myContract.feedAndMultiply(0, 21, "kitty", { from: accounts[0] });
    });

    it("Test req ZFE_1 (false)", async function() {
      await catchRevert(myContract.feedAndMultiply(0, 21, "kitty", { from: accounts[1] }));
    }); 
  });
});
...
contract("ZombieFeeding 2", function(accounts) {
  ...
  it("Make Initial Zombies, Test ZFE_2, ZFE_3, ZFE_4", async function() {
    ...
    .feedAndMultiply(0, 21, "kitty", { from: accounts[0] })
    .then(function(result) {
      for (var i = 0; i < result.logs.length; i++) {
        var log = result.logs[i];
        if (log.event == "NewZombie") {
    ...
    let tx4 = await instance
    .feedAndMultiply(0, 21, "not kitty", { from: accounts[0] })
    .then(function(result) {
      for (var i = 0; i < result.logs.length; i++) {
        var log = result.logs[i];
        if (log.event == "NewZombie") {
          ...
          d = d.toNumber();
          var e = d % 100;
          console.log("Test ZFE_3 (false) Id of new Zombie: ", n, "Name: ", s, "dna: ", d); 
          assert.notEqual(e, 99, "Should NOT be kitty dna");
        }
      }
    });
  });
